...
class SkylineFullSongTransformerDataset(Dataset):
    ...
    for seg in range(batch_n_segs):
      for samp_idx in range(len(batch)):
        if 'dec_inp_{}'.format(seg) in batch[samp_idx]:
          ...
          batch[samp_idx]['dec_inp_{}'.format(seg)] = np.full((self.model_dec_seqlen,), fill_value=self.pad_token)
          batch[samp_idx]['dec_tgt_{}'.format(seg)] = np.full((self.model_dec_seqlen,), fill_value=self.pad_token)
  ...
  def build_dataset(self):
    if not self.pieces:
      ...
      self.pieces = sorted( [
      os.path.join(self.data_dir, p + '.pkl') for p in self.pieces \
      if os.path.exists(os.path.join(self.data_dir, p + '.pkl'))
    ])
  ...
  def get_decoder_input_data(self, piece_idx, bar_positions, piece_events, st_seg, ed_seg):
    inp_segments = []
    tgt_segments = []
    segments_bar_pos = []
    segments_len = []
    ...
    for seg, (seg_st_bar, seg_ed_bar) in enumerate(sample_segs):
      ...
      inp_segments.append( 
        np.array(piece_events[seg_st_idx : seg_ed_idx])
      )
      tgt_segments.append(
        np.array(piece_events[seg_st_idx + 1 : seg_ed_idx + 1])
      )
    ...
    return inp_segments, tgt_segments, segments_bar_pos, segments_len
  ...
  def get_encoder_input_data(self, piece_idx, bar_positions, piece_events, st_seg, ed_seg, ext_enc_bar_evs=None):
    ...
    for seg, (seg_st_bar, seg_ed_bar) in enumerate(sample_segs):
      ...
      for b, (st, ed) in enumerate(zip(seg_bar_positions[:-1], seg_bar_positions[1:])):
        ...
        seg_chroma_mask[b, :], seg_groove_mask[b, :] = compute_feature_masks(piece_events[st : ed], self.idx2event, segment_max_bar_len)

        within_bar_events = self.pad_or_truncate_sequence(piece_events[st : ed], segment_max_bar_len, self.pad_token)
  ...
  def __getitem__(self, idx):
    ...
    data_dict = dict()
    dec_inp, dec_tgt, dec_bar_positions, dec_seg_len = self.get_decoder_input_data(
