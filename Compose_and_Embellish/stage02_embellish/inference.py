...
out_dir = sys.argv[3]
n_pieces = int(sys.argv[4]) if len(sys.argv) > 4 else 20  # Number of pieces to generate
samp_per_piece = int(sys.argv[5]) if len(sys.argv) > 5 else 1  # Samples per piece
...
print(f'[info] generating {n_pieces} pieces with {samp_per_piece} sample(s) per piece')
...
if __name__ == '__main__':
  ...
  # Filter out keys that don't match the model architecture
  pretrained_dict = {
    k:v for k, v in pretrained_dict.items() 
    if 'feature_map.omega' not in k 
