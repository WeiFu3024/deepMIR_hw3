...
def parse_args():
    ...
    parser.add_argument('--model_path', type=str, required=True,
                        help='path to load/store model checkpoint')
    parser.add_argument('--load_model_loss', type=int, default=None,
                        help='loss value of the model checkpoint to load')
    parser.add_argument('--gen_dir', type=str, default='gen_midis',
                        help='directory to save generated midis')
    parser.add_argument('--num_songs', type=int, default=20,
                        help='number of songs to generate during inference')
...
def sampling(logit, p=None, t=1.2):
...
class CausalTransformerEncoder(nn.Module):
    """Standard PyTorch Transformer Encoder with causal masking"""
    def __init__(self, d_model, n_head, n_layers, d_ff=2048, dropout=0.1):
    ...
    def forward(self, x, attn_mask=None):
        ...
        return self.transformer_encoder(x, mask=attn_mask)


class RecurrentTransformerEncoder(nn.Module):
    """Recurrent/Incremental Transformer Encoder for inference that matches training structure"""
    def __init__(self, d_model, n_head, n_layers, d_ff=2048, dropout=0.1):
        ...
        # Match the training model structure by wrapping in TransformerEncoder
        encoder_layer = nn.TransformerEncoderLayer(
            d_model=d_model,
            nhead=n_head,
            dim_feedforward=d_ff,
            dropout=dropout,
            activation='gelu',
            batch_first=True  # Important: match training model
        )
        self.transformer_encoder = nn.TransformerEncoder(encoder_layer, num_layers=n_layers)
        
    def forward(self, x, memory=None):
        # x: (batch, 1, d_model) for single step inference
        # memory: accumulated past context (batch, seq_len, d_model)
        
        if memory is None:
            # Initialize memory with current input
            memory = x
        else:
            # Concatenate new input with memory
            memory = torch.cat([memory, x], dim=1)
        
        # Create causal mask for the accumulated sequence
        seq_len = memory.size(1)
        attn_mask = torch.triu(torch.ones(seq_len, seq_len, device=x.device), diagonal=1).bool()
        
        # Forward through transformer encoder
        output = self.transformer_encoder(memory, mask=attn_mask)
        
        # Take only the last position output
        h = output[:, -1:, :]
        
        return h.squeeze(1), memory  # Return (batch, d_model) and updated memory
...
class TransformerModel(nn.Module):
    def __init__(self, n_token, is_training=True):
        ...
        if is_training:
            ...
            # encoder (inference)
            print(' [o] using recurrent backend.')
            self.transformer_encoder = RecurrentTransformerEncoder(
                d_model=self.d_model,
                n_head=self.n_head,
                n_layers=self.n_layer,
                d_ff=2048,
        ...
        # calculate params
        total_params = network_paras(self)
        trainable_params = sum(p.numel() for p in self.parameters() if p.requires_grad)
        print(f' [o] Total model params: {total_params}')
        print(f' [o] Trainable model params: {trainable_params}')
    ...
    def forward_hidden(self, x, memory=None, is_training=True):
        ...
        x.shape=(bs, nf) for inference or (bs, seq_len, nf) for training
        '''
        ...
        if is_training:
            ...
            h = self.transformer_encoder(pos_emb, attn_mask=None)
            y_type = self.proj_type(h)
            return h, y_type
        else:
            # Inference mode - incremental generation
...
if __name__ == '__main__':
    ...
    if args.load_model_loss is not None:
        info_load_model = (
            # path to ckpt for loading
            args.model_path,
            # loss
            args.load_model_loss                               
        )
    else:
    ...
    path_gendir = args.gen_dir
    num_songs = args.num_songs
    MODE = args.mode
    path_exp = args.model_path
